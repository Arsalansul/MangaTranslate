...
_OCR_HAN_VERT = _OCR_HAN + (
    "\n- Набор вертикальный, колонки читаются справа налево; порядок уже\n"
    "  восстановлен, но соседняя колонка иногда прилипает к реплике или\n"
    "  теряется. Если фраза обрывается на полуслове — переводи то, что есть,\n"
    "  не досочиняя продолжение."
)
...
SOURCES = {
    ...
    "chi_sim_vert": {"name": "китайского (упрощённого, вертикального)",
                     "ocr": _OCR_HAN_VERT},
    "chi_tra_vert": {"name": "китайского (традиционного, вертикального)",
                     "ocr": _OCR_HAN_VERT},
}
